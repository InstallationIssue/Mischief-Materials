    ...
    return (
      <div className='flex justify-center items-center h-full aspect-square relative font-play'>
        ...
        transition={{ duration: 0.5 }}>
          ...
          <svg viewBox="0 0 200 200" className='h-full aspect-square absolute -rotate-90' version="1.1" xmlns="http://www.w3.org/2000/svg">
            <motion.circle id='back' r="90" cx="100" cy="100" className='fill-transparent stroke-danger-light stroke-[5%]'
            strokeDasharray={total_length}
            initial={{ strokeDashoffset: total_length }}
            animate={{ strokeDashoffset: 0 }}
            transition={{ duration: 0.8 }}/>
            <motion.circle id="bar" r="90" cx="100" cy="100" className='fill-transparent stroke-success-light stroke-[10%]' 
            strokeDasharray={total_length} strokeDashoffset={-offset} strokeLinecap='round' initial={{ strokeDashoffset: total_length }}
            animate={{ strokeDashoffset: offset }}
            transition={{ duration: 1 }}/>
          </svg>  
        </motion.div>
        <motion.div className='absolute left-0 bottom-0 h-2/5 flex items-center justify-center'
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
        transition={{ duration: 1 }}>
