...
export default async function MagicCard({
    id, name, medium, spells
  }: {
    id: number, name: string, medium: string, spells: number
  }) {
    ...
    return (
        <Link href={`/magic/${id}`} className="flex flex-row gap-2 h-12 magics-center overflow-clip border rounded-lg p-2 border-primary-light items-center font-header text-lg">
            ...
            {medium == 'electricity' && <Electricity className='h-8 w-8 fill-primary-light'/>}
            {medium == 'golem' && <Golem className='h-8 w-8 fill-primary-light'/>}
            {medium == 'homunculus' && <Homunculus className='h-8 w-8 fill-primary-light'/>}
